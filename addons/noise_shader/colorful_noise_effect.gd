...
class_name ColorfulNoiseEffect
## Adds a noisy effect to a camera.


const NOISE_SHADER_PATH: String = "res://addons/noise_shader/colorful_noise.glsl"
...
@export var value_steps: int = 3:
	set(value):
		value_steps = maxi(value, 2)
## How fast each value of the noise is cycled through.
@export var value_speed: float = 0.2:
	set(value):
		value_speed = clampf(value, 0.0, 1.0)
## How many different hues there are.
@export var hue_steps: int = 3:
	set(value):
		hue_steps = maxi(value, 2)
## How fast each hue of the noise is cycled through.
@export var hue_speed: float = 0.1:
	set(value):
		hue_speed = clampf(value, 0.0, 1.0)
## How much the hue is offset by.
@export_range(0.0, 1.0) var hue_offset: float = 0.0
...
## Assigns a random float to each value of the noise.
...
## Assigns a float of 0.0 to each value and hue of the noise.
...
## Assigns a value of 0.5 to each value of the noise.
func white_out_noise() -> void:
	for offset: int in range(0, _write_data.size(), 8):
		_write_data.encode_float(offset, 0.5)
	for offset: int in range(0, _read_data.size(), 8):
		_read_data.encode_float(offset, 0.5)
...
func _render_callback(effect_callback_type: int, render_data: RenderData) -> void:
	...
	if not _buffer_size == size.x * size.y * BYTES_PER_BUFFER_FLOAT * 2:
		_buffer_size = size.x * size.y * BYTES_PER_BUFFER_FLOAT * 2
		_update_storage_set()
...
func _get_push_constant(size: Vector2) -> PackedByteArray:
	...
	push_constant.encode_s32(8, value_steps)
	push_constant.encode_float(12, value_speed)
	push_constant.encode_s32(16, hue_steps)
	push_constant.encode_float(20, hue_speed)
	push_constant.encode_float(24, hue_offset)
